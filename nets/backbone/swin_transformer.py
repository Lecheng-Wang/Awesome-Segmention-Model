...
# @Author  ：Lecheng Wang
# @Time    : 2025/03/16 17:18
# @Function: Realization of swin-transformer architecture
...
def Swin_Transformer(img_size=224, wind_size=7, in_chans=3, num_classes=1000, backbone='Swin_T', **kwargs):
    configs = {
    ...
    model  = SwinTransformer(img_size=img_size, in_chans=in_chans, num_classes=num_classes, window_size=wind_size,
                            embed_dim=config["embed_dim"],depths=config["depths"],num_heads=config["num_heads"],
                            patch_size=4,mlp_ratio=4.,qkv_bias=True,qk_scale=None,drop_rate=0.,attn_drop_rate=0.,
...
if __name__ == "__main__":
    ...
    model           = Swin_Transformer(img_size=224, wind_size=7, in_chans=3, num_classes=1000, backbone='Swin_T').to(device)
